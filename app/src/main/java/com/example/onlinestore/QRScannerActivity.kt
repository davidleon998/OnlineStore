...
class QRScannerActivity : AppCompatActivity() {
    ...
    private fun handleQRCode(qrContent: String) {
        ...
        // Si el QR contiene el esquema personalizado, abrir el formulario de opinión
        if (qrContent == "onlinestore://opinionform" || qrContent.contains("opinionform")) {
            val intent = Intent(this, OpinionFormActivity::class.java)
            startActivity(intent)
        } else {
            // Para otros códigos QR, mostrar mensaje
            Toast.makeText(
                this,
                "Código QR escaneado: $qrContent",
                Toast.LENGTH_LONG
            ).show()
        }
        finish()
